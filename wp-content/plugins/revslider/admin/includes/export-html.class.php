...
class RevSliderSliderExportHtml extends RevSliderSliderExport {
	...
	public function export_slider_html($slider_id){
		...
		if($this->slider->get_param('prem', false) === true && $this->_truefalse($this->get_options(['system', 'valid'], 'false')) === false){
			echo __('Wrong request!', 'revslider');
			exit;
		}
...
<html lang="en-US" class="no-js">
	...
	public function write_footer_html(){
		...
		if(!empty($SR_GLOBALS['collections']['css'])){
			...
			echo '<style>';
			echo $this->compress_css($custom_css);
			echo '</style>'."\n";
		}
	...
	public function remove_json_from_html(){
		...
		$json		= str_replace(['<!-- TEMPJSON -->', '<!-- /TEMPJSON -->'], '', $json);
	...
	public function replace_export_html_urls(){
		...
		$search = [$cont_url, $cont_url_no_www, RS_PLUGIN_URL];
		...
		foreach($search ?? [] as $s){
			...
			foreach($found ?? [] as $_file){
				$o		= $_file;
				$_file	= str_replace(['"', "'", str_replace('/', '\/', $s)], '', $_file);
				...
				if(is_file($upload_dir.$_file)){
					$this->add_html_file_to_zip($upload_dir.$_file, $use_path_raw);
					$remove = true;
				}elseif(is_file($upload_dir_multi.$_file)){
					$this->add_html_file_to_zip($upload_dir_multi.$_file, $use_path_raw);
					$remove = true;
				}elseif(is_file(RS_PLUGIN_PATH.$_file)){
					//we need to be special with internal files
					$__file = basename($_file);
					$this->add_html_file_to_zip($use_path_raw.'/'.$__file, $use_path_raw);
					$remove = true;
					$add = '/';
				}else{
					if(defined('WHITEBOARD_PLUGIN_PATH') && is_file(WHITEBOARD_PLUGIN_PATH.$_file)){
						...
						$__file = basename($_file);
						$this->add_html_file_to_zip(WHITEBOARD_PLUGIN_PATH.$__file, $use_path_raw);
						$remove = true;
						$add = '/';
					}
					foreach($replace ?? [] as $_path){
						...
						$__file = basename($_file);
						$this->add_html_file_to_zip($_path.$__file, $use_path_raw);
						$remove = true;
						$add = '/';
					}
				}
				...
				$this->slider_html = str_replace([$re.$o.$re, $re.'http:'.$o.$re, $re.'https:'.$o.$re], $to, $this->slider_html);
				$this->slider_html = str_replace([$re.$_o.$re, $re.'http:'.$_o.$re, $re.'https:'.$_o.$re], $_to, $this->slider_html);
			}
		}
		...
		$notice_text = __('The use, distribution, or reproduction of this data is permitted only with a valid license for the WordPress Slider Revolution plugin. License details and verification are available at: https://www.themepunch.com/links/slider_revolution_wordpress_regular_license', 'revslider');
		
		if(!$this->usepcl){
			$this->zip->addFromString('NOTICE.txt', $notice_text); //add slider settings
		}else{
			$this->pclzip->add([[PCLZIP_ATT_FILE_NAME => 'NOTICE.txt', PCLZIP_ATT_FILE_CONTENT => $notice_text]]);
		}
	}
	...
	public function add_export_json_to_zip(){
		if(!$this->usepcl){
			...
			$this->pclzip->add([[PCLZIP_ATT_FILE_NAME => "assets/".$this->html_id.".json", PCLZIP_ATT_FILE_CONTENT => $this->json]]);
		}
	}
	...
	public function add_export_html_to_zip(){
		if(!$this->usepcl){
			...
			$this->pclzip->add([[PCLZIP_ATT_FILE_NAME => 'index.html', PCLZIP_ATT_FILE_CONTENT => $this->slider_html]]);
		}
	}

	public function add_html_file_to_zip($file, $path){
