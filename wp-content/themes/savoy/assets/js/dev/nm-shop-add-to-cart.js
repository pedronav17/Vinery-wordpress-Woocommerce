			...
			self.$body.on('click', '.single_add_to_cart_button', function(e) {
				...
				if ($thisButton.hasClass('nm-simple-add-to-cart-button') || $thisButton.hasClass('nm-variable-add-to-cart-button')) {
					...
					if (self.quickviewOpen) {
                        ...
                        setTimeout(function() {
                            var $quickView = $('#nm-quickview');
                        
                            $quickView.addClass('fade-out');
                            ...
                            setTimeout(function() {
                                ...
                                setTimeout(function() {
                                    $quickView.removeClass('fade-out');
                                }, 200);
                            }, 200);
                        }, 200);
					}
			...
			$.ajax({
				...
				success: function(response) {
					...
					var fragments = {
                        ...
                        '.widget_shopping_cart_content': $response.find('.widget_shopping_cart_content').prop('outerHTML') // Cart panel
					};
					...
					if (self.quickviewOpen) {
						...
						if (hasError) {
							...
							if (self.$cartPanel.length) {
								...
								setTimeout(function() {
									self.cartPanelShow(false, true); // Args: showLoader, addingToCart
								}, 350);
							}
						}*/
					} else {
						...
						if (hasError) {
							setTimeout(function() {
								// Hide cart panel and overlay
								$('#nm-cart-panel-overlay').trigger('click');
