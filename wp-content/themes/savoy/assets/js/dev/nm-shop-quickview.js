		...
		quickview_init: function() {
			...
			var _qvLoadProduct = function() {
				...
				window.nm_quickview_get_product = $.ajax({
					...
					success: function(data) {
                        // Cancel if empty string is returned (product is likely Private or Unpublished)
                        if (data.length === 0) {
                            console.log('NM: AJAX - _qvLoadProduct() - Empty data');
			...
			_qvInitSlider = function() {
				...
				if (self.$qvSlider.children().length > 1) {
                    ...
                    var infinite = (nm_wp_vars.quickViewGalleryInfinite == '1') ? true : false;
