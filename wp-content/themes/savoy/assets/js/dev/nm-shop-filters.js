	...
	$.extend($.nmTheme, {
		...
		shopFiltersBind: function() {
            ...
            if (self.$shopSidebarPopupBtn.length) {
                ...
                /* Bind: Filters popup menu - Close button */
				$('#nm-shop-sidebar-popup-close-button').on('click', function(e) {
					e.preventDefault();
            ...
            /* Bind: Results bar - Individual active filter reset links */
			if (self.filtersEnableAjax && self.$pageIncludes.hasClass('shop_filters')) {
                self.$shopWrap.on('click.nmShopIndividualFilterReset', '.nm-shop-results-bar a:not(.nm-shop-reset-button)', function(e) {
                    e.preventDefault();
					self.shopGetPage($(this).attr('href'));
                });
            }
        ...
        shopFiltersPopupHide: function() {
            ...
            setTimeout(function() {
                self.shopFiltersPopupButtonToggle(); // Show popup button (if it should be visible)
		...
		shopGetPage: function(pageUrl, isBackButton, isProductTag) {
			...
			if (pageUrl) {
				...
				if (self.$body.width() < self.filterPanelHideWidth) {
                    ...
                    // Hide active filter panel?
                    if (nm_wp_vars.shopFiltersMobileAutoClose !== '0') {
                        self.shopFiltersHideActivePanel();
                    }
		...
		shopUpdateContent: function(ajaxHTML) {
            ...
            if (self.$body.hasClass('nm-shop-scroll-enabled')) {
                // Smooth-scroll to top
                var to = self.shopScrollToTop();
                setTimeout(function() {
                    // Hide 'loader' overlay (after scroll animation)
