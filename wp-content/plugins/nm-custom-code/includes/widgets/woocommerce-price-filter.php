...
class NM_WC_Widget_Price_Filter extends WC_Widget {
	...
	public function widget( $args, $instance ) {
        ...
        // Note: Using intval() below since prices can return null (causes PHP deprecation notice)
		$prices = $this->get_filtered_price();
		$min    = floor( intval( $prices->min_price ) );
		$max    = ceil( intval( $prices->max_price ) );
        ...
        $nofollow_attributes = apply_filters( 'nm_price_filter_nofollow_attributes', true );
        $nofollow_attribute = ( $nofollow_attributes ) ? ' rel="nofollow"' : '';
        
		$output = '<ul class="nm-price-filter">';
		        
		if ( strlen( $min_price ) > 0 ) {
			$output .= '<li><a href="' . esc_url( $link ) . '"' . $nofollow_attribute  . '>' . esc_html__( 'All', 'nm-framework' ) . '</a></li>';
		} else {
		...
		for ( $range_min = 0; $range_min < ( $max + $range_size ); $range_min += $range_size ) {
			...
			if ( $count == $max_ranges ) {
				...
				if ( $range_min != $min_price ) {
                    ...
                    $output .= '<li><a href="' . esc_url( $url ) . '"' . $nofollow_attribute  . '>' . $price_output . '</a></li>';
				} else {
