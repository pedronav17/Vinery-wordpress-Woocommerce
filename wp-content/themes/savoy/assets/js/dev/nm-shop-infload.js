	...
	$.extend($.nmTheme, {
		...
		shopInfLoadGetPage: function() {
			...
			var $nextPageLink = self.$shopBrowseWrap.children('.nm-infload-link').find('a'),
				$infloadControls = self.$shopBrowseWrap.children('.nm-infload-controls'),
                $infloadButtonWrap = $infloadControls.children('.nm-infload-controls-button'),
			...
			if (nextPageUrl) {
				...
				// Show 'loader'
				$infloadControls.addClass('loading');
                $infloadButtonWrap.addClass('nm-loader');
				...
				self.shopAjax = $.ajax({
					...
					complete: function() {
						// Hide 'loader'
						$infloadControls.removeClass('loading');
                        $infloadButtonWrap.removeClass('nm-loader');
					},
					success: function(response) {
                        ...
                        var $newResultCount = $response.find('.woocommerce-result-count');
                        if ($newResultCount.length) {
                            $infloadControls.children('.woocommerce-result-count').replaceWith($newResultCount);
                        }
