<?php
	...
	function nm_shortcode_team( $atts, $content = NULL ) {
        ...
        $args = apply_filters( 'nm_shortcode_team_args', $args );
		...
		while ( $team->have_posts() ) : $team->the_post();
			...
			if ( $member_meta ) {
				$icon_names = apply_filters( 'nm_team_members_icon_names', array(
					...
					'nm_team_member_instagram'		=> 'instagram-filled',
					'nm_team_member_twitter'		=> 'x-twitter',
        ...
        $grid_columns_class = 'xsmall-block-grid-2 small-block-grid-2 medium-block-grid-2 large-block-grid-' . esc_attr( $columns );
        $grid_columns_class = apply_filters( 'nm_shortcode_team_grid_columns_class', $grid_columns_class );
        
		$output = '
			<ul class="nm-team ' . esc_attr( $grid_columns_class ) . ' text-align-' . esc_attr( $text_alignment ) . '">' .
				$output . '
			</ul>';
