 ...
 * @version 9.4.0
 NM: Modified - 1. Modified $show_more variable, 2. Added "All" index link, 3. Added wrapper container, 4. Included brand thumbnail */

declare( strict_types = 1);

$show_empty = apply_filters( 'nm_brands_list_show_empty', false );
$nm_index_title = apply_filters( 'nm_brands_list_index_title', '<i class="nm-font nm-font-menu"></i>' );
?>
<div id="brands_a_z">

	<ul class="brands_index">
        <li class="current-index"><a href="#all"><?php echo wp_kses_post( $nm_index_title ); ?></a></li>
		<?php
		foreach ( $index as $i ) {
			...
			<ul class="brands">
				...
				foreach ( $product_brands[ $i ] as $brand ) {
					printf(
						...
						esc_html( $brand->name ),
                        wc_get_brand_thumbnail_image( $brand )
					);
				}
				?>
			</ul>
