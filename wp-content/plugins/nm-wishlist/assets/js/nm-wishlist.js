(function($) {
	...
	var NM_Wishlist = {
		...
		init: function() {
            ...
            self.isSetUp        = false;
            self.wishlistIds    = [];
            self.cookieName     = 'nm-wishlist-ids',
            ...
            if (nm_wishlist_vars.wlAddLocaleSuffix != '0') {
            ...
            if (nm_wishlist_vars.wlMenuCount != '0') {
                ...
                self.$counts = $('.nm-menu-wishlist-count');
            } else {
        ...
        setUp: function() {
            ...
            if (self.isLoggedIn) { 
                ...
                $.ajax({
                    ...
                    success: function(json) {
                        if (json.ids) {
                            ...
                            Cookies.set(self.cookieName, JSON.stringify(idsArray), { expires: self.cookieExpires });
                ...
                if (undefined !== Cookies.getJSON) { // Note: Cookies.getJSON was removed from "Cookies" library in a recent version
                    var ids = Cookies.getJSON(self.cookieName);
                } else {
                    var ids = Cookies.get(self.cookieName);
                ...
                if (! ids) {
                    Cookies.set(self.cookieName, '[]', { expires: self.cookieExpires });
                } else {
        ...
        IDsUpdate: function() {
            ...
            Cookies.set(self.cookieName, ids, { expires: self.cookieExpires });
        ...
        countUpdate: function() {
            ...
            if (self.countEnabled) {
                if (self.wishlistIds.length) {
                    ...
                    self.$counts.text(self.wishlistIds.length).removeClass('is-zero');
                } else {
                    self.$headerLinkLi.removeClass('has-items');
                    self.$counts.text('0').addClass('is-zero');
                }
            }
        }
	}
